...
class UserControllerTest {
    ...
    void validateFail(){
        ...
        assertEquals(exception, "Пустое поле с логином");

        final User userFail2 = new User(2, "aiarkhipova@yandex.ru", "la la",
                LocalDateTime.now().minusYears(22));
        Exception exception2 = assertThrows(ValidationException.class, () -> userController.validate(userFail2));
        assertEquals(exception2, "В логине присутствуют пробелы");

        final User userFail3 = new User(3, "aiarkhipovayandex.ru", "",
                LocalDateTime.now().minusYears(22));
        Exception exception3 = assertThrows(ValidationException.class, () -> userController.validate(userFail3));
        assertEquals(exception3, "В электронной почте нет символа собачки");

        final User userFail4 = new User(4, "", "",
                LocalDateTime.now().minusYears(22));
        Exception exception4 = assertThrows(ValidationException.class, () -> userController.validate(userFail4));
        assertEquals(exception4, "Пустое поле с электронной почтой");

        final User userFail5 = new User(5, "aiarkhipova@yandex.ru", "",
        ...
        assertEquals(exception5, "День рождения пользователя в будущем");
    }
}